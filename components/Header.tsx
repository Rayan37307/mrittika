...
function IconButton({
  ...
  return (
    <button
      type="button"
      aria-label={label}
      className="relative flex items-center justify-center h-9 w-9 text-ink hover:text-terracotta transition-colors cursor-pointer"
    >
      {children}
      {typeof count === "number" && (
    ...
    <header className="sticky top-0 z-40 bg-cream/95 backdrop-blur border-b hairline">
      <div className="container-px flex items-center justify-between gap-4 py-4">
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <IconButton label="Wishlist" count={2}>
            <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none">
              <path
          ...
          <IconButton label="Cart, 0 items — $0.00" count={0}>
            <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none">
              <path
